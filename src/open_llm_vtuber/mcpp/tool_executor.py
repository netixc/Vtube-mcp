...
from .subprocess_mcp_client import SubprocessMCPClient


class ToolExecutor:
    def __init__(
        self,
        mcp_client: Union[MCPClient, SubprocessMCPClient],
    ...
    async def execute_tools(
        ...
        logger.info(f"🏁 Tool results for LLM: {tool_results_for_llm}")
    ...
    async def run_single_tool(
        ...
        if not tool_info:
            ...
            try:
                logger.info(f"🚀 Calling MCP tool '{tool_name}' on server '{tool_info.related_server}'")
                result_dict = await self._mcp_client.call_tool(
                    server_name=tool_info.related_server,
                    tool_name=tool_name,
                    tool_args=tool_input,
                )
                logger.info(f"📨 Got result from MCP tool")
